...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        in_features = img_height * img_width * channels
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        for i in range(batch_size):
            input = torch.flatten(state[i])
            output = self.q_network(input) if network == 'q_network' else self.target_network(input)
            q_value_vectors.append(output)
        out = torch.stack(q_value_vectors)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        q_samp = rewards + torch.bitwise_not(done_mask)*gamma*torch.max(target_q_values, dim=1)[0]
        actions_bitmask = torch.nn.functional.one_hot(actions.to(torch.int64), num_classes=num_actions)
        q_s_a = torch.sum(q_values * actions_bitmask, 1)
        return torch.nn.functional.mse_loss(q_samp, q_s_a)
        ##############################################################
        ######################## END YOUR CODE #######################
